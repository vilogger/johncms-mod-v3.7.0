...
class counters
{
    ...
    */
    static function forum_new($mod = 0)
    {
        if (core::$user_id) {
            ...
            if ($mod) {
                return '<a href="/forum/index.php?act=new&amp;do=period">' . core::$lng['show_for_period'] . '</a>' .
                ($total ? '<br/><a href="/forum/index.php?act=new">' . core::$lng['unread'] . '</a>&#160;<span class="red">(<b>' . $total . '</b>)</span>' : '');
            } else {
                return $total;
            }
        } else {
            if ($mod) {
                return '<a href="/forum/index.php?act=new">' . core::$lng['last_activity'] . '</a>';
            } else {
                return false;
            }
        }
    ...
    */
    ...
    {
        $users = mysql_result(mysql_query("SELECT COUNT(*) FROM `users` WHERE `lastdate` > '" . (time() - 30) . "'"), 0);
        $guests = mysql_result(mysql_query("SELECT COUNT(*) FROM `cms_sessions` WHERE `lastdate` > '" . (time() - 30) . "'"), 0);
        return (core::$user_id || core::$system_set['active'] ? '<a rel="nofollow" href="' . core::$system_set['homeurl'] . '/users/index.php?act=online">' . $users . ' / ' . $guests . '</a>' : core::$lng['online'] . ': ' . $users . ' / ' . $guests);
    }
