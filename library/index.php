...
if ($do) {
    ...
    $hdr = $id > 0 ? html_entity_decode(mb_substr(mysql_result(mysql_query("SELECT `name` FROM `" . $tab . "` WHERE `id`=" . $id . " LIMIT 1"), 0), 0, 30), ENT_QUOTES, 'UTF-8') : '';
        ...
        if ($total) {
            while ($row = mysql_fetch_assoc($sql)) {
                ...
                echo '<div class="list' . (++$i % 2 ? 2 : 1) . '">'
                    . '<a href="?do=dir&amp;id=' . $row['id'] . '">' . functions::checkout(html_entity_decode($row['name'], ENT_QUOTES, 'UTF-8')) . '</a> ('
            ...
            default:
                ...
                if ($res['premod'] || $adm) {
                    ...
                    echo $text .
                        ...
                        '</div>';
                    echo '<div class="alarm">Đánh giá sao:</div>'.$nav
                        . ($user_id && $page == 1 ? $rate->print_vote() : '');
                    if($user_id){
                        echo '<div class="phdr">Thăm dò ý kiến.</div>';
...
if (mysql_num_rows($req_obj)) {
    ...
    $arg = array(
        ...
        'script'         => '?binhluan',         // Имя скрипта (с параметрами вызова)
