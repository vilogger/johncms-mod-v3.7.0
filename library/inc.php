    ...
    */ 
    public function print_nav_panel()
    {
        ...
        foreach ($array as $k => $v) {
            $return[] = $x == $cnt ? '<strong>' . functions::checkout(html_entity_decode($v, ENT_QUOTES, 'UTF-8')) . '</strong>' : '<a href="?do=dir&amp;id=' . $k . '">' . functions::checkout(html_entity_decode($v, ENT_QUOTES, 'UTF-8')) . '</a>';
            $x++;
        }
