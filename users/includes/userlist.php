...
$req = mysql_query("SELECT `id`, `name`, `sex`, `lastdate`, `datereg`, `facebook_ID`, `google_ID`, `status`, `rights`, `ip`, `browser`, `rights` FROM `users` WHERE `preg` = 1 ORDER BY `datereg` DESC LIMIT $start, $kmess");
for ($i = 0; ($res = mysql_fetch_assoc($req)) !== false; $i++) {
    echo '<div class="list1" '.($res['facebook_ID'] ? 'style="background: #fff url(/images/facebook.png) no-repeat right top;" ' : ($res['google_ID'] ? 'style="background: #fff url(/images/googlep.png) no-repeat right top;" ' : '')).'>';
