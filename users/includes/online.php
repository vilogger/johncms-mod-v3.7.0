...
if ($total) {
    ...
    while (($res = mysql_fetch_assoc($req)) !== false) {
        ...
        else echo '<div class="list1" '.($res['facebook_ID'] ? 'style="background: #fff url(/images/facebook.png) no-repeat right top;" ' : ($res['google_ID'] ? 'style="background: #fff url(/images/googlep.png) no-repeat right top;" ' : '')).'>';
        $arg['stshide'] = 1;
