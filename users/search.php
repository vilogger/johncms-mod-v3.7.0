...
echo '<div class="phdr"><a href="index.php"><b>' . $lng['community'] . '</b></a> | ' . $lng['search_user'] . '</div>' .
    ...
    '<div class="gmenu"><p>' .
    '<input type="text" name="search" value="' . functions::checkout($inputsearch) . '" />' .
...
*/
$error = array();
if (!empty($search) && (mb_strlen($search) < 2 || mb_strlen($search) > 40))
...
if ($search && !$error) {
    ...
    */
    ...
    if ($total > $kmess) {
        echo '<div class="topmenu">' . functions::display_pagination('search.php?search=' . urlencode($inputsearch) . '&amp;', $start, $total, $kmess) . '</div>' .
             '<p><form action="search.php?search=' . urlencode($inputsearch) . '" method="post">' .
