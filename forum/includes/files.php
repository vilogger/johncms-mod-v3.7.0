...
if ($do || isset($_GET['new'])) {
    ...
    */
    ...
    $reqp = mysql_query("SELECT `seo` FROM `forum` WHERE `id` = '$res[topic]'");
    $resp = mysql_fetch_array($reqp);
            $text = '<b><a href="/forum/' . $res['topic'] . '/' . $resp['seo'] . '_p' . $page . '.html">' . $res['topicname'] . '</a></b><br />' . $text;
            if (mb_strlen($res['text']) > 500)
                $text .= '<br /><a href="/forum/post-' . $res['post'] . '.html">' . $lng_forum['read_all'] . ' &gt;&gt;</a>';
            ...
            if (in_array($att_ext, $pic_ext)) {
                ...
                $file = '<div style="padding: 4px 4px;"><table><tr><td><a href="index.php?act=file&amp;id=' . $res['id'] . '">';
                $file .= '<img src="thumbinal.php?file=' . (urlencode($res['filename'])) . '" alt="' . $lng_forum['click_to_view'] . '" /></a></td><td><a href="index.php?act=file&amp;id=' . $res['id'] . '">' . htmlspecialchars($res['filename']) . '</a><br /><span class="gray">' . $lng_forum['size'] . ': ' . $fls . ' kb.<br />' . $lng_forum['downloaded'] . ': ' . $res['dlcount'] . ' lượt</span></td></tr></table></div>';
            } else {
                // Если обычный файл, выводим значок и ссылку
                $file = '<div style="padding: 4px 4px;">';
            ...
            $file .= '<span class="gray">' . $lng_forum['size'] . ': ' . $fls . ' kb.<br />' . $lng_forum['downloaded'] . ': ' . $res['dlcount'] . ' lượt</span>';
                $file .= '</div>';
            }
            $arg = array(
                'iphide' => 1,
    ...
    */
    ...
    $reqpt = mysql_query("SELECT `seo` FROM `forum` WHERE `id` = '$id'");
    $respt = mysql_fetch_array($reqpt);
echo '<p>' . (($do || isset($_GET['new']))
        ? '<a href="index.php?act=files' . $lnk . '">' . $lng_forum['section_list'] . '</a><br />'
        : '') . '<a href="/forum' . ($id ? '/' . $id . '/' . $respt['seo'] . '.html' : '') . '">' . $lng['forum'] . '</a></p>';
