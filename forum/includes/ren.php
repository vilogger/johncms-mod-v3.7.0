...
if ($rights == 3 || $rights >= 6) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $seo = @addslashes(functions::seourl($_POST['nn']));
        ...
        mysql_query("update `forum` set  text='" . $nn . "', tags='" . mysql_real_escape_string($tags) . "', seo='" . $seo . "' where id='" . $id . "';");
        header('Location: /forum/'.$id.'/'.$seo.'.html');
    } else {
        /*
        -----------------------------------------------------------------
        ...
        */
        require('../incfiles/head.php');
        echo '<div class="phdr"><a href="/forum/' . $id . '/' . $ms['seo'] . '.html"><b>' . $lng['forum'] . '</b></a> | ' . $lng_forum['topic_rename'] . '</div>' .
            '<div class="menu"><form action="index.php?act=ren&amp;id=' . $id . '" method="post">' .
            '<p><h3>' . $lng_forum['topic_name'] . '</h3><br />' .
            '<input type="text" name="nn" value="' . $ms['text'] . '"/></p>' .
        '<p><h3>Tags:</h3><br /><input type="text" name="tags" value="' . $ms['tags'] . '" /> </p>' .
            '<p><input type="submit" name="submit" value="' . $lng['save'] . '"/></p>' .
            '</form></div>' .
            '<div class="phdr"><a href="/forum/' . $id . '/' . $ms['seo'] . '.html">' . $lng['back'] . '</a></div>';
    }
} else {
    require('../incfiles/head.php');
