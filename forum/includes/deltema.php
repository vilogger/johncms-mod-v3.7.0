...
if ($rights == 3 || $rights >= 6) {
    ...
    $r_i = $res['refid'];
    $r_d = mysql_query("SELECT * FROM `forum` WHERE `id` = '$r_i'");
    $r_d1 = mysql_fetch_assoc($r_d);
    if (isset($_POST['submit'])) {
        ...
        header('Location: /forum/'.$r_d1['id'].'/'.$r_d1['seo'].'.html');
    } else {
        /*
        -----------------------------------------------------------------
        ...
        */
        require('../incfiles/head.php');
        echo '<div class="phdr"><a href="/forum/' . $id . '/'.$res['seo'].'.html"><b>' . $lng['forum'] . '</b></a> | ' . $lng_forum['topic_delete'] . '</div>' .
             '<div class="rmenu"><form method="post" action="index.php?act=deltema&amp;id=' . $id . '">' .
             '<p><h3>' . $lng['delete_confirmation'] . '</h3><br />' .
             ...
             '<p><a href="/forum/' . $id . '/'.$res['seo'].'.html">' . $lng['cancel'] . '</a>' .
             '</p></form></div>' .
             '<div class="phdr">&#160;</div>';
    }
} else {
