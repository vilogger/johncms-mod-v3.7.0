    ...
    switch ($do) {
            ...
            */
            ...
            if ($count > 0) {
                ...
                for ($i = 0; $res = mysql_fetch_array($req); ++$i) {
                    ...
                    $q3 = mysql_query("SELECT `id`, `refid`, `text`, `seo` FROM `forum` WHERE `type`='r' AND `id`='" . $res['refid'] . "'");
                    ...
                    echo '&#160;<a href="/forum/' . $res['id'] . '/' . $res['seo'] . ($cpg > 1 && $set_forum['upfp'] && $set_forum['postclip'] ? '_clip' : '') . ($set_forum['upfp'] && $cpg > 1 ? '_p' . $cpg : '') . '.html">' . $res['text'] .
                        '</a>&#160;[' . $colmes1 . ']';
                    if ($cpg > 1) {
                        echo '<a href="/forum/' . $res['id'] . '/' . $res['seo'] . (!$set_forum['upfp'] && $set_forum['postclip'] ? '_clip' : '') . ($set_forum['upfp'] ? '' : '_p' . $cpg) . '.html">&#160;&gt;&gt;</a>';
                    }

                    echo '<br /><div class="sub"><a href="/forum/' . $razd['id'] . '/' . $razd['seo'] . '.html">' . $frm['text'] . '&#160;/&#160;' . $razd['text'] . '</a><br />';
                    echo $res['from'];
        ...
        default:
            ...
            */
            ...
            echo '<div class="phdr"><a href="index.html"><b>' . $lng['forum'] . '</b></a> | ' . $lng['unread'] . '</div>';
            if ($total > $kmess)
            ...
            if ($total > 0) {
                ...
                for ($i = 0; $res = mysql_fetch_assoc($req); ++$i) {
                    if ($res['close'])
                        ...
                        echo $i % 2 ? '<div class="list2">' : '<div class="list1">';
                    $q3 = mysql_query("SELECT `id`, `refid`, `text`, `seo` FROM `forum` WHERE `type` = 'r' AND `id` = '" . $res['refid'] . "' LIMIT 1");
                    $razd = mysql_fetch_assoc($q3);
                    $q4 = mysql_query("SELECT `id`, `text`, `seo` FROM `forum` WHERE `type`='f' AND `id` = '" . $razd['refid'] . "' LIMIT 1");
                    ...
                    // Значки
                    $icons = array(
                        (isset($np) ? (!$res['vip'] ? functions::image('np.gif') : '') : functions::image('op.gif')),
                    ...
                    echo '<div class="sub">' . $res['from'] . ($colmes1 > 1 ? '&#160;/&#160;' . $nick['from'] : '') .
                        ...
                        '<a href="/forum/' . $frm['id'] . '/' . $frm['seo'] . '.html">' . $frm['text'] . '</a>&#160;/&#160;<a href="/forum/' . $razd['id'] . '/' . $razd['seo'] . '.html">' . $razd['text'] . '</a>' .
                        '</div></div>';
                }
            } else {
    ...
    */
    ...
    if (mysql_num_rows($req)) {
        for ($i = 0; $res = mysql_fetch_assoc($req); ++$i) {
            $q3 = mysql_query("select `id`, `refid`, `text`, `seo` from `forum` where type='r' and id='" . $res['refid'] . "' LIMIT 1");
            $razd = mysql_fetch_assoc($q3);
            $q4 = mysql_query("select `id`, `refid`, `text`, `seo` from `forum` where type='f' and id='" . $razd['refid'] . "' LIMIT 1");
            $frm = mysql_fetch_assoc($q4);
            ...
            echo functions::display_menu($icons, '');
            echo '<a href="/forum/' . $res['id'] . '/' . $res['seo'] . '.html">' . $res['text'] . '</a>&#160;[' . $colmes1 . ']';
            if ($cpg > 1)
                echo '&#160;<a href="/forum/' . $res['id'] . '/' . $res['seo'] . '_clip_p' . $cpg . '.html">&gt;&gt;</a>';
            echo '<br/><div class="sub"><a href="/forum/' . $razd['id'] . '/' . $razd['seo'] . '.html">' . $frm['text'] . '&#160;/&#160;' . $razd['text'] . '</a><br />';
            echo $res['from'];
            if (!empty($nam['from'])) {
                echo '&#160;/&#160;' . $nam['from'];
