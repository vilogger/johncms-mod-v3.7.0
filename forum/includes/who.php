...
if ($id) {
    ...
    -----------------------------------------------------------------
    */
    $req = mysql_query("SELECT `text`, `seo` FROM `forum` WHERE `id` = '$id' AND `type` = 't'");
    if (mysql_num_rows($req)) {
        $res = mysql_fetch_assoc($req);
        echo '<div class="phdr"><b>' . $lng_forum['who_in_topic'] . ':</b> <a href="/forum/' . $id . '/' . $res['seo'] . '.html">' . $res['text'] . '</a></div>';
        if ($rights > 0){
    ...
    $reqt = mysql_query("SELECT `text`, `seo` FROM `forum` WHERE `id` = '$id' AND `type` = 't'");
    $rest = @mysql_fetch_assoc($reqt);
    echo '<p><a href="/forum/' . $id . '/'.$rest['seo'].'.html">' . $lng_forum['to_topic'] . '</a></p>';
} else {
    /*
    -----------------------------------------------------------------
            ...
            switch ($res['place']) {
                ...
                default:
                    ...
                    if ($where[0] == 'forum' && intval($where[1])) {
                        $req_t = mysql_query("SELECT `type`, `refid`, `text`, `seo` FROM `forum` WHERE `id` = '$where[1]'");
                        if (mysql_num_rows($req_t)) {
                            $res_t = mysql_fetch_assoc($req_t);
                            $link = '<a href="/forum/' . $where[1] . '/' . $res_t['seo'] . '.html">' . $res_t['text'] . '</a>';
                            switch ($res_t['type']) {
                                case 'f':
                                ...
                                case 'm':
                                    $req_m = mysql_query("SELECT `text`, `seo` FROM `forum` WHERE `id` = '" . $res_t['refid'] . "' AND `type` = 't'");
                                    if (mysql_num_rows($req_m)) {
                                        $res_m = mysql_fetch_assoc($req_m);
                                        $place = (isset($where[2]) ? $lng_forum['place_answer'] : $lng_forum['place_topic']) . ' &quot;<a href="/forum/' . $res_t['refid'] . '/' . $res_m['seo'] . '.html">' . $res_m['text'] . '</a>&quot;';
                                    }
                                    break;
                            }
                        }
