...
$seof = mysql_query("SELECT * FROM `forum` WHERE `id` = '".$res['refid']."'");
$seof1 = mysql_fetch_assoc($seof);
                $curators = !empty($seof1['curators']) ? unserialize($seof1['curators']) : array();
                $curator = false;
                if ($rights < 6 && $rights != 3 && $user_id) {
                    if (array_key_exists($user_id, $curators)) $curator = true;
                }
if ($res['type'] != 'm' || ($res['user_id'] != $user_id && !$curator && $rights < 7)) {
    echo functions::display_error($lng['error_wrong_data']);
    require('../incfiles/end.php');
    exit;
}
...
if (isset($_POST['submit'])) {
    ...
    */
    ...
    $pa = mysql_query("SELECT * FROM `forum` WHERE `type` = 'm' AND `refid` = '" . $res['refid'] . "'");
    ...
    -----------------------------------------------------------------
    */
    echo '<div class="phdr">' . $lng_forum['add_file'] . ' | <a href="index.php?act=import&amp;id=' . $id . '">Nhập khẩu file</a></div>' .
