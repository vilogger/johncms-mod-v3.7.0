...
if (mysql_num_rows($req)) {
    ...
    */
    ...
    $link = '/forum/' . $res['refid'] . '/'.$topic['seo'].'_p' . $page . '.html';
    $error = FALSE;
    ...
    $error = $lng_forum['error_post_deleted'] . '<br /><a href="index.html">' . $lng['forum'] . '</a>';
}
if (!$error) {
    switch ($do) {
        case 'restore':
            ...
            */
            ...
            if ($rights == 9 && !isset($_GET['hide'])) {
                ...
                if ($posts < 2) {
                    ...
                    header('Location: /forum/' . $res['refid'] . '/' . $topic['seo'] . '_p' . $page . '.html');
                }
            } else {
                // Скрытие поста
                $req_f = mysql_query("SELECT * FROM `cms_forum_files` WHERE `post` = '$id' LIMIT 1");
                ...
                if ($posts == 1) {
                    ...
                    $topic_l = mysql_fetch_assoc(mysql_query("SELECT * FROM `forum` WHERE `id` = " . $res_l['refid']));
                    mysql_query("UPDATE `forum` SET `close` = '1', `close_who` = '$login' WHERE `id` = '" . $res['refid'] . "' AND `type` = 't'");
                    header('Location: /forum/' . $res_l['refid'] . '/'.$topic_l['seo'].'.html');
                } else {
        ...
        default:
            ...
            */
            ...
            if ($youtube_id){
                $msg = $msg.' [youtube]'.$youtube_id.'[/youtube]';
            }
            ...
            if (isset($_POST['submit'])) {
                ...
                mysql_query("UPDATE `forum` SET
                    ...
                    WHERE `id` = '$id'
                ");
                header('Location: /forum/' . $res['refid'] . '/'.$topic['seo'].'_p' . $page . '.html#post'.$id.'');
            } else {
                ...
                echo '<div class="rmenu"><div class="story-publisher-box"><form name="form" action="?act=editpost&amp;id=' . $id . '&amp;start=' . $start . '" method="post"><p>';
                echo bbcode::auto_bb('form', 'msg');
                echo '<textarea class="box-edit" name="msg">' . (empty($msg) ? htmlentities($res['text'], ENT_QUOTES, 'UTF-8') : functions::checkout($msg)) . '</textarea><br/>';
