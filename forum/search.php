...
function ReplaceKeywords($search, $text)
{
    ...
    return mb_strlen($search) < 3 ? $text : preg_replace('|(' . preg_quote($search, '/') . ')|siu', '<h3>$1</h3>', $text);
}

function ReplaceKeywordstitle($search, $text)
{
    $search = str_replace('*', '', $search);
    return mb_strlen($search) < 3 ? $text : preg_replace('|(' . preg_quote($search, '/') . ')|siu', '<span class="titlesearch">$1</span>', $text);
}
...
switch ($act) {
    ...
    default:
        ...
        -----------------------------------------------------------------
        */
        $search_post = isset($_POST['search']) ? functions::check(trim($_POST['search'])) : false;
        $search_get = isset($_GET['search']) ? rawurldecode(trim($_GET['search'])) : false;
        $search = $search_post ? $search_post : $search_get;
        $inputsearch = $search_post ? trim($_POST['search']) : $search_get;
        //$search = preg_replace("/[^\w\x7F-\xFF\s]/", " ", $search);
        $search_t = isset($_REQUEST['t']);

        $to_history = false;
        echo '<div class="gmenu"><form action="search.php" method="post"><p>' .
             '<input type="text" value="' . ($search ? htmlentities($inputsearch, ENT_QUOTES, 'UTF-8') : '') . '" name="search" />' .
            ...
            if ($total) {
                ...
                while (($res = mysql_fetch_assoc($req)) !== false) {
                    ...
                    if (!$search_t) {
                        ...
                        $req_t = mysql_query("SELECT `id`,`text`, `seo` FROM `forum` WHERE `id` = '" . $res['refid'] . "'");
                        $res_t = mysql_fetch_assoc($req_t);
                        echo '<b>' . $res_t['text'] . '</b><br />';
                    } else {
                        // Поиск в названиях тем
                        $req_p = mysql_query("SELECT `text`, `seo` FROM `forum` WHERE `refid` = '" . $res['id'] . "' ORDER BY `id` ASC LIMIT 1");
                        $res_p = mysql_fetch_assoc($req_p);
                        foreach ($array as $val) {
                            $res['text'] = ReplaceKeywordstitle($val, $res['text']);
                        }
                    ...
                    if (mb_strlen($res['text']) > 500)
                        echo '...<a href="/forum/post-' . $res['id'] . '.html">' . $lng_forum['read_all'] . ' &gt;&gt;</a>';
                    echo '</div><div class="list1" style="background-color: #fcfcfc;"><a href="/forum/' . ($search_t ? $res['id'] : $res_t['id']) . '/' . ($search_t ? $res['seo'] : $res_t['seo']) . '.html">' . $lng_forum['to_topic'] . '</a>' . ($search_t ? ''
                            : ' | <a href="/forum/post-' . $res['id'] . '.html">' . $lng_forum['to_post'] . '</a>');
                    echo '</div>';
                    ++$i;
                }
            } else {
