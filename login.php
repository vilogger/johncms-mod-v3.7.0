...
if(core::$user_id){
    ...
    echo '<div class="phdr">' . $lng['login'] . ' | <a href="registration.php">' . $lng['registration'] . '</a></div>';
    $error = array();
    $captcha = FALSE;
    $display_form = 1;
    ...
    if ($display_form) {
        ...
        echo $info;
        echo '<div class="menu">Đăng nhập với: <a href="/facebook_login/"><img src="/images/fb.png" /></a> <a href="/google_login/"><img src="/images/googlep.png" /></a> <img src="/images/twitter.png" /></div>';
        echo '<div class="gmenu"><form action="login.php" method="post"><p>' . $lng['login_name'] . ':<br/>' .
            ...
            '<div class="mdn-group block-group">
                <label class="mdn-switch">
                    <input type="checkbox" name="mem" checked="checked" />
                    <span class="switch-toggle"></span>
                    <span class="switch-label">' . $lng['remember'] . '</span>
                </label>
            </div>'.
            '<p><input type="submit" value="' . $lng['login'] . '"/> &#160;<a href="users/skl.php?continue">' . $lng['forgotten_password'] . '?</a></p>' .
            '</form></div>' .
            '<div class="menu"><p><a href="registration.php">' . $lng['registration'] . '</a> nếu bạn chưa có tài khoản.</p></div>';
    }
}
