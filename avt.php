...
$ida = htmlspecialchars(urldecode($_GET['avt']));
$usr = functions::get_user($ida);

$radius = isset($_GET['bo']) ? htmlspecialchars(urldecode($_GET['bo'])) : NULL;
$size = isset($_GET['size']) ? htmlspecialchars(urldecode($_GET['size'])) : NULL;

if (file_exists(($rootpath.'files/users/avatar/' . $ida . '_100x100.' . $usr['avatar_extension']))) {
    $file = 'files/users/avatar/'.$ida.'_100x100.' . $usr['avatar_extension'];
} else {
    $file = 'images/default-'.($usr['sex'] == 'm' ? 'male' : 'female').'-avatar.png';
}
...
if ($file && file_exists($file)) {
    ...
    if ($att_ext) {
        ...
        if(($att_ext == 'jpg' || $att_ext == 'jpeg' || $att_ext == 'png') && $radius > 1 && $radius <= 100)
{
            $img = $tmp;
            $rate = 3;
