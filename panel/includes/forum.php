            ...
            if ($total) {
                if ($res['type'] == 'f') {
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        echo '<div class="rmenu"><p><h3>' . $lng_forum['category_deleted'] . '</h3><br />' . $lng_forum['contents_moved_to'] . ' <a href="../forum/index.php?id=' . $category . '">' . $lng_forum['selected_category'] . '</a></p></div>';
                    } else {
                        echo '<form action="index.php?act=forum&amp;mod=del&amp;id=' . $id . '" method="POST">' .
                            '<div class="rmenu"><p>' . $lng['contents_move_warning'] . '</p>' .
                            '<p><h3>' . $lng_forum['select_category'] . '</h3><br /><select name="category" size="1">';
                        ...
                        if ($rights == 9) {
                            ...
                            echo '<div class="rmenu"><p><h3>' . $lng_forum['delete_full'] . '</h3><br />' . $lng_forum['delete_full_note'] . ' <a href="index.php?act=forum&amp;mod=cat&amp;id=' . $id . '">' . $lng_forum['child_section'] . '</a></p>' .
                                '</div>';
                        }
                        echo '</form>';
                    }
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        mysql_query("DELETE FROM `forum` WHERE `id` = '$id'");
                        echo '<div class="rmenu"><p><h3>' . $lng_forum['section_deleted'] . '</h3><br />' . $lng_forum['themes_moved_to'] . ' <a href="../forum/index.php?id=' . $subcat . '">' . $lng_forum['selected_section'] . '</a>.' .
                            '</p></div>';
                        ...
                        echo '<form action="index.php?act=forum&amp;mod=del&amp;id=' . $id . '" method="POST"><div class="rmenu">' .
                            '<p>' . $lng_forum['section_move_warning'] . '</p>' . '<p><h3>' . $lng_forum['select_section'] . '</h3><br />';
                        ...
                        echo '</p><p><h3>' . $lng_forum['another_category'] . '</h3><br /><ul>';
                        ...
                        if ($rights == 9) {
                            // Для супервайзоров запрос на полное удаление
                            echo '<div class="rmenu"><p><h3>' . $lng_forum['delete_full'] . '</h3><br />' . $lng_forum['delete_full_warning'];
        ...
        */
        ...
        if (isset($_POST['submit'])) {
            ...
            echo '<form action="index.php?act=forum&amp;mod=add' . ($id ? '&amp;id=' . $id : '') . '" method="post">' .
                '<div class="gmenu">' .
                '<p><h3>' . $lng['title'] . '</h3><br />' .
                '<input type="text" name="name" />' .
                '<br /><small>' . $lng['minmax_2_30'] . '</small></p>' .
                '<p><h3>' . $lng['description'] . '</h3><br />' .
            ...
            if ($res['type'] == 'f' || $res['type'] == 'r') {
                if (isset($_POST['submit'])) {
                    ...
                    if ($res['type'] == 'r') {
                        ...
                        echo '<p><h3>' . $lng_forum['category'] . '</h3><br /><select name="category" size="1">';
        ...
        if ($id) {
            ...
            if (mysql_num_rows($req)) {
                ...
                while ($res = mysql_fetch_assoc($req)) {
                    ...
                    echo '<b>' . $res['text'] . '</b>' .
                        '&#160;<a href="../forum/' . $res['id'] . '/' . $res['seo'] . '.html">&gt;&gt;</a>';
                    if (!empty($res['soft']))
    ...
    default:
        ...
        */
        ...
        echo '<div class="phdr"><a href="index.php"><b>' . $lng['admin_panel'] . '</b></a> | ' . $lng_forum['forum_management'] . '</div>' .
            '<div class="gmenu"><p><h3>' . functions::image('rate.gif') . $lng['statistics'] . '</h3><br /><ul>' .
            ...
            '</ul></p></div>' .
            '<div class="menu"><p><h3><img src="../images/settings.png" width="16" height="16" class="left" />&#160;' . $lng['settings'] . '</h3><br /><ul>' .
