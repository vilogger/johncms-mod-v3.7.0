...
switch ($act) {
        ...
        if ($rights >= 6 && $id) {
            if (isset($_POST['submit'])
                ...
                echo '<div class="menu">' .
                    ...
                    '<p><h3>' . $lng['reply'] . '</h3><br />' . bbcode::auto_bb('form', 'otv') .
    ...
    default:
        ...
        */
        ...
        echo '<div class="phdr"><b>Phòng chat</b></div>';
        echo '<div class="rmenu">Đăng nhập để được phép chém gió.!</div>';
        }
        if (isset($_SESSION['ga']) && $rights >= "1") {
        ...
        if ($total) {
            ...
            for ($i = 0; $res = mysql_fetch_assoc($req); ++$i) {
                ...
                $text = '<div align="right" style="font-size: x-small;"><span class="gray">(' . functions::display_date($res['time']) . ')</span></div>';
                if ($res['user_id']) {
                ...
                if ($res['edit_count']) {
                    ...
                    $post .= '<br /><span class="gray" style="font-size: 12px;">Chỉnh sửa: <b>' . $res['edit_who'] . '</b> (' . functions::display_date($res['edit_time']) . ') <b>[' . $res['edit_count'] . ']</b></span>';
                }
                if (!empty($res['otvet'])) {
                    // Ответ Администрации
                    ...
                    $post .= '<div class="reply"><b>' . $res['admin'] . '</b>: <span style="font-size: 12px;">(' . functions::display_date($res['otime']) . ')</span><br/>' . $otvet . '</div>';
                }
                if ($rights >= 6) {
