    ...
    if ($total) {
        ...
        for ($i = 0; ($row = mysql_fetch_assoc($req)) !== FALSE; ++$i) {
            if ($row['read'] == 0 && $row['from_id'] == $user_id) {
                $mass_read[] = $row['id'];
                $out .= '<div class="list1" style="background-color: #fffdcc; border-color: #ffda19;">';
            }else{
                $out .= '<div class="list1">';
            }
            $out .= '<table cellpadding="0" cellspacing="0"><tr><td style="padding: 2px 4px 0px 0px;" valign="top">' . functions::image('f/'.$row['them'].'.png', array('class' => 'icon-tb')) . '</td><td>';
            $post = $row['text'];
            ...
            $out .= $post;
            $out .= '</td></tr></table>';
            $out .= '<div class="sub"><span style="margin: 0px 0px 0px 28px;font-size: x-small; color: gray;">' . functions::display_date($row['time']) . '</span><span class="rightf" style="font-size: x-small;"><a href="index.php?act=delete&amp;id=' . $row['id'] . '">' . $lng['delete'] . '</a></span></div>';
            $out .= '</div>';
        }
...
echo '<a href="../users/profile.php">' . $lng['personal'] . '</a></p>';