...
`time` = '" . time() . "';");
    ...
    LIMIT $start, $kmess"
            ...
            for ($i = 0; ($row = mysql_fetch_assoc($req)) !== FALSE; ++$i) {
                ...
                $subtext = '<a href="index.php?act=write&amp;id=' . $row['id'] . '">' . $lng_mail['correspondence'] . ($new_count_message > 0 ? ' <span style="color: #000;">(</span><span style="color: red;">'.$new_count_message.'</span><span style="color: #000;">)</span>' : '') . '</a> | <a href="index.php?act=deluser&amp;id=' . $row['id'] . '">' . $lng['delete'] . '</a> | <a href="index.php?act=ignor&amp;id=' . $row['id'] . '&amp;add">' . $lng_mail['ban_contact'] . '</a>';
                $arg = array(
                    'sub' => $subtext
                );
